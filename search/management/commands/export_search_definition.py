...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        root_path = path.join(options['export_dir'], options['search_id'])
        ...
        # Export Fields
        dataset = ExportFieldResource(options['search_id']).export()
        field_path = path.join(db_path, "{0}_fields.json".format(options['search_id']))
        with open(field_path, 'w', encoding='utf-8', errors="ignore") as search_file:
            search_file.write(dataset.json)
        logging.info("Fields exported to {0}".format(field_path))

        # Export Codes
        dataset = ExportCodeResource(options['search_id']).export()
        code_path = path.join(db_path, "{0}_codes.json".format(options['search_id']))
